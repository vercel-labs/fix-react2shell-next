  ...
  try {
    ...
    if (content.packages) {
      for (const [packagePath, info] of Object.entries(content.packages)) {
        ...
        if (packageName && REACT_PACKAGES.includes(packageName) && info.version) {
          packages.push({
            name: packageName,
    ...
    if (cves.length > 0) {
      ...
      for (const cve of cves) {
        if (cve.patchedVersion) {
          if (!highestVersion || 
              (parseVersion(cve.patchedVersion) && 
               parseVersion(highestVersion) &&
               parseVersion(cve.patchedVersion).major > parseVersion(highestVersion).major) ||
              (parseVersion(cve.patchedVersion).major === parseVersion(highestVersion).major &&
               parseVersion(cve.patchedVersion).minor > parseVersion(highestVersion).minor) ||
              (parseVersion(cve.patchedVersion).major === parseVersion(highestVersion).major &&
               parseVersion(cve.patchedVersion).minor === parseVersion(highestVersion).minor &&
               parseVersion(cve.patchedVersion).patch > parseVersion(highestVersion).patch)) {
