...
const { findAndScanLockfiles } = require("./utils/lockfile-scanner");
...
function analyzePackageJson(pkgPath) {
  ...
  for (const packageName of packagesToCheck) {
    ...
    let shouldCheckInstalled =
      isUnparseableVersionSpec(version) ||
      !parseVersion(version) ||
      hasRangeSpecifier(version);
    ...
    if (
      affectedCves.length === 0 &&
      isUnparseableVersionSpec(allDeps[packageName]) &&
      !installedVersion
    ) {
      vulnerablePackages.push({
        ...
        cves: [
          { id: "UNKNOWN", severity: "unknown", patchedVersion: "15.5.8" },
        ],
        patched: "15.5.8",
    ...
    for (const vuln of file.vulnerabilities) {
      ...
      for (const cve of vuln.cves) {
        if (cve.patchedVersion) {
          if (
            !highestVersion ||
            compareVersions(cve.patchedVersion, highestVersion) > 0
          ) {
...
async function run() {
  ...
  const scanLockfiles =
    args.includes("--scan-lockfiles") || args.includes("--lockfiles");

  if (!jsonOutput) {
    console.log(
      "\n" +
        c("bold", "fix-react2shell-next") +
        c("dim", " - Next.js vulnerability scanner\n")
    );
    console.log(
      c(
        "dim",
        `Checking for ${vulnerabilities.length} known vulnerabilities:\n`
      )
    );
    for (const vuln of vulnerabilities) {
      const severityColor =
      ...
      console.log(
        c("dim", "  - ") +
          c(severityColor, vuln.id) +
          c("dim", ` (${vuln.severity}): ${vuln.description}`)
      );
    }
    console.log();
  }
  ...
  if (packageJsonPaths.length === 0) {
    if (jsonOutput) {
      console.log(
        JSON.stringify({ vulnerable: false, reason: "no-package-json" })
      );
    } else {
      console.log(
        c("yellow", "No package.json files found in current directory.\n")
      );
    }
    return;
  }
  ...
  if (!jsonOutput) {
    console.log(
      c("dim", `Found ${packageJsonPaths.length} package.json file(s)\n`)
    );
  }
  ...
  // Scan lock files if requested
  let lockfileVulnerabilities = [];
  if (scanLockfiles) {
    if (!jsonOutput) {
      console.log(
        c("cyan", "🔒 Scanning lock files for React 19 vulnerabilities...\n")
      );
    }
    lockfileVulnerabilities = findAndScanLockfiles(cwd);

    if (!jsonOutput && lockfileVulnerabilities.length > 0) {
      console.log(
        c(
          "red",
          `Found ${lockfileVulnerabilities.length} lock file(s) with vulnerable React packages:\n`
        )
      );

      for (const lockfile of lockfileVulnerabilities) {
        const relativePath =
          path.relative(cwd, lockfile.path) || lockfile.lockfile;
        console.log(c("yellow", `  ${relativePath}`));

        for (const vuln of lockfile.vulnerabilities) {
          const cveList = ` [${vuln.cves.join(", ")}]`;
          console.log(
            c("dim", `     ${vuln.package}: `) +
              c("red", vuln.currentVersion) +
              c("dim", " -> ") +
              c("green", vuln.patchedVersion || "?") +
              c("magenta", cveList)
          );
        }
        console.log();
      ...
      console.log(
        c(
          "cyan",
          "💡 To fix lock file issues, update your package.json versions and run your package manager's install command.\n"
        )
      );
    }
  }
  ...
  if (jsonOutput) {
    console.log(
      JSON.stringify(
        {
          vulnerable:
            minimalFixes.length > 0 || lockfileVulnerabilities.length > 0,
          count: minimalFixes.length,
          files: minimalFixes,
          lockfiles: lockfileVulnerabilities,
        },
        null,
        2
      )
  ...
  if (minimalFixes.length === 0) {
    ...
    console.log(
      c("dim", "  Your project is not affected by any known vulnerabilities.\n")
    );
    return;
  }
  ...
  if (modifiedDirs.length === 0) {
    console.log(
      c(
        "yellow",
        "   No files were modified (patches may require manual intervention).\n"
      )
    );
    return;
  }
  ...
  if (allInstallsSucceeded) {
    if (lockfileOnly) {
      ...
      console.log(
        c(
          "dim",
          "   Run your package manager's install command to download the updated packages."
        )
      );
    } else {
      console.log(c("green", "\nPatches applied!"));
    ...
    console.log(
      c("dim", "   Remember to test your app and commit the changes.\n")
    );
  } else {
    console.log(
      c(
    ...
    console.log(
      c(
        "dim",
        "   Please run install commands manually in the affected directories.\n"
      )
    );
    if (lockfileOnly) {
      process.exit(1);
