...
function getInstalledVersionFromPackageManager(pkgDir, packageName) {
  ...
  try {
    if (packageManager === 'pnpm') {
      ...
      const result = spawnSync('yarn', ['info', packageName, '--json'], {
        cwd: pkgDir,
        encoding: 'utf8',
        shell: process.platform === 'win32',
      });
      if (result.status === 0 && result.stdout) {
        // yarn info outputs newline-delimited JSON
      ...
      // bun doesn't have great JSON output, fall through to node_modules check
    }
  } catch (e) {
